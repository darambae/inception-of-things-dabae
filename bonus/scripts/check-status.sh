...
echo "Attente du pod GitLab... (max ${TIMEOUT_SEC}s)"
start_time=$(date +%s)

while true; do
    STATUS=$(kubectl get pods -n gitlab -l app=gitlab \
    ...
    if [ "$elapsed" -ge "$TIMEOUT_SEC" ]; then
        echo "❌ Timeout : GitLab n'est pas prêt après ${TIMEOUT_SEC}s"
        exit 1
    fi

    echo "🔄 Pas encore prêt... (${elapsed}s écoulées)"
    sleep "$INTERVAL"
done