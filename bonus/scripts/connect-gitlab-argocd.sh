...
kubectl exec -i -n gitlab deploy/gitlab -- env PUB_KEY="$PUB_KEY" gitlab-rails runner "
...
GITLAB_USER="root"
GITLAB_PASS=$(kubectl exec -n gitlab deploy/gitlab -- \
    gitlab-rake "gitlab:password:reset[root]" 2>/dev/null | grep "Password:" | awk '{print $2}')
